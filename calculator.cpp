...
void drawBorder(int x=0, int y=0){
    ...
    gotoxy((width/2)+x-11, y); cout<<"YOUR CURRENT AGE IS ";
...
int main() {
	...
	do{
		...
		cout<<"Provide Information Regarding Your Age:"<<endl;
		cout<<"Enter Day: ";
		cin>>day;
		cout<<"Enter Month: ";
		cin>>month;
		...
		gotoxy(posX+gap, 13); cout<<"Month(s)";
		if( month < 10 ){
		...
		printDigit(10, posX+=gap, posY);
		gotoxy(posX+gap+2, 13); cout<<"Day(s)";
		if( day < 10 ){
			printDigit(0, posX+=gap, posY);
		...
		gotoxy(15, 17);
		cout<<"Do you want to calculate age again (Y/N): ";
		op = getche();
